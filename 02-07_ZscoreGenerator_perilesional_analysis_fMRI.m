...
for zscore_computation = 1
    ...
    for control_computation = 1
        ...
        for fMRI_read_file = 1
            ...
            fMRI_ALFF_temp = zeros(size(case_num_cont, 1), vertexnum);
            fMRI_ReHo_temp = zeros(size(case_num_cont, 1), vertexnum);
            ...
            for j = 1 : size(case_num_cont, 1)
                ...
                prefix_path = [ data_dir '/' Prefix_cont '_' case_num_cont{j} '/' Prefix_cont '_' case_num_cont{j} '_z' ];                
        ...
        for fMRI_zscore = 1
            ...
            %% z-score
            fMRI_ALFF_z_cont = zeros(size(case_num_cont, 1), vertexnum);
            fMRI_ReHo_z_cont = zeros(size(case_num_cont, 1), vertexnum);
...
%             fMRI_ReHo_z_cont(logical(outlier_vector), :) = [];
    ...
    for patient_computation = 1
        ...
        for fMRI_read_file = 1
            ...
            fMRI_ALFF_temp2 = zeros(size(case_num_pat, 1), vertexnum);
            fMRI_ReHo_temp2 = zeros(size(case_num_pat, 1), vertexnum);
           
            data_dir = '/local_raid/seokjun/01_project/05_NoelRest/03_Result/01_DPARSF_processing/DPARSFA_scrub/02_without_mean_sig_regout/bbr_rest/';
            file_postfix = [ '_aniso_sm_' num2str(Kernel) '_rsl.txt' ];
            for j = 1 : size(case_num_pat, 1)
                fprintf([ case_num_pat{j} ' : ' ]);
                prefix_path = [ data_dir '/' Prefix_pat '_' case_num_pat{j} '/' Prefix_pat '_' case_num_pat{j} '_z' ];                
        ...
        for fMRI_zscore = 1
            ...
            %% z-score
            fMRI_ALFF_z = zeros(size(case_num_pat, 1), vertexnum);
            fMRI_ReHo_z = zeros(size(case_num_pat, 1), vertexnum);
            ...
            for j = 1 : size(case_num_pat, 1)
                ...
                %% Intensity-based features: corrected RI, pg, tg
                fMRI_ALFF_z(j, 1:vertexnum) = (fMRI_ALFF_temp2(j, 1:vertexnum) - fMRI_ALFF_mean) ./ fMRI_ALFF_std;
                fMRI_ReHo_z(j, 1:vertexnum) = (fMRI_ReHo_temp2(j, 1:vertexnum) - fMRI_ReHo_mean) ./ fMRI_ReHo_std;
                fprintf([ 'z-score of fMRI: ALFF, ReHo\n' ]);
            end
            fMRI_ALFF_z(find(isinf(fMRI_ALFF_z))) = 0;
            fMRI_ReHo_z(find(isinf(fMRI_ReHo_z))) = 0;
            
        end
        
        % save zscore as a mat file
        for save_zscore = 1
            if(~exist([ 'zscore_database_perilesional_fMRI_sm_' num2str(Kernel) '.mat'], 'file'))
                save( [ 'zscore_database_perilesional_fMRI_sm_' num2str(Kernel) '.mat'], ...
                    'fMRI_ALFF_z', ...                    
                    'fMRI_ReHo_z');
            end
        end
