...
echo PerpendicularGradientCalculator_T1_FLAIR_ratio'(' "'"${OUTDIR}"'", "'"${PREFIX}"'", "'"${ID}"'", "'"${LEFT_RIGHT}"'", ${NUMMESH}, ${NUMINSURF}, ${NUMSUBSURF}, "'"${SAMPLING_SPACE}"'", ${PVE_CORR}, {"'"t1_flair_ratio"'"} ')'
matlab -nodisplay << EOF > ${OUTDIR}/${ID}/logs/${PREFIX}_${ID}_${LEFT_RIGHT}_${NUMMESH}_perpedicular_gradient_ratio.log


  addpath( '${EMMADIR}' );
  addpath( '${SURFSTATDIR}' ); 
  
  PerpendicularGradientCalculator_T1_FLAIR_ratio( '${OUTDIR}', ...
		    						 							  '${PREFIX}', ...
		 														  '${ID}', ...
									      					  '${LEFT_RIGHT}', ...
  									         					${NUMMESH}, ...
