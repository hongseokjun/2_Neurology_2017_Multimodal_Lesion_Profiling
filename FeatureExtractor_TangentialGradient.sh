...
echo TangentialGradientCalculator'(' "'"${OUTDIR}"'", "'"${PREFIX}"'", "'"${ID}"'", "'"${LEFT_RIGHT}"'", ${NUMMESH}, ${NUMINSURF}, ${NUMSUBSURF}, "'"${SAMPLING_SPACE}"'", ${PVE_CORR}, {"'"t1"'", "'"flair"'"} ')'
matlab -nodisplay << EOF > ${OUTDIR}/${ID}/logs/${PREFIX}_${ID}_${LEFT_RIGHT}_${NUMMESH}_tangential_gradient.log


  addpath( '${EMMADIR}' );
  addpath( '${SURFSTATDIR}' ); 
  
  TangentialGradientCalculator( '${OUTDIR}', ...
		    					 		  '${PREFIX}', ...
		 								  '${ID}', ...
								        '${LEFT_RIGHT}', ...
  								         ${NUMMESH}, ...
