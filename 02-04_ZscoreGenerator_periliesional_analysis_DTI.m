...
for zscore_computation = 1
    ...
    for control_computation = 1
        ...
        for DTI_zscore = 1
            ...
            %% z-score
            DTI_FA_IntCortical_z_cont         = zeros(NumIntSurf+2, vertexnum, size(case_num_cont, 1));
            DTI_FA_SubCortical_z_cont         = zeros(NumSubSurf,   vertexnum, size(case_num_cont, 1));
            DTI_MD_IntCortical_z_cont         = zeros(NumIntSurf+2, vertexnum, size(case_num_cont, 1));
            DTI_MD_SubCortical_z_cont         = zeros(NumSubSurf,   vertexnum, size(case_num_cont, 1));
            ...
            for j = 1 : size(case_num_cont, 1)
                ...
                %% Intensity-based features: corrected RI, pg, tg
                DTI_FA_IntCortical_z_cont(:, 1:vertexnum, j) = (DTI_FA_IntCortical_temp(:, 1:vertexnum, j) - DTI_FA_IntCortical_mean) ./ DTI_FA_IntCortical_std;
                DTI_MD_IntCortical_z_cont(:, 1:vertexnum, j) = (DTI_MD_IntCortical_temp(:, 1:vertexnum, j) - DTI_MD_IntCortical_mean) ./ DTI_MD_IntCortical_std;
                fprintf([ 'z-score of FA and MD in Inc, ' ]);
                
                %% Intensity-based features of subcortical surfaces: RI, pg, tg
                DTI_FA_SubCortical_z_cont(:, 1:vertexnum, j) = (DTI_FA_SubCortical_temp(:, 1:vertexnum, j) - DTI_FA_SubCortical_mean) ./ DTI_FA_SubCortical_std;
                DTI_MD_SubCortical_z_cont(:, 1:vertexnum, j) = (DTI_MD_SubCortical_temp(:, 1:vertexnum, j) - DTI_MD_SubCortical_mean) ./ DTI_MD_SubCortical_std;
                fprintf([ 'z-score of FA and MD in Sub\n' ]);
            end
            
            DTI_FA_IntCortical_z_cont(:, :, logical(outlier_vector)) = [];
            DTI_FA_SubCortical_z_cont(:, :, logical(outlier_vector)) = [];
            DTI_MD_IntCortical_z_cont(:, :, logical(outlier_vector)) = [];
            DTI_MD_SubCortical_z_cont(:, :, logical(outlier_vector)) = [];
