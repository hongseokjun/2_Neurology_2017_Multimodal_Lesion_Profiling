...
for zscore_computation = 1
    ...
    for control_computation = 1
        ...
        for T1_read_file = 1
            ...
            data_dir = '/local_raid/seokjun/01_project/04_IntracorticalAnalysis/03_result/validation_pg/';

            file_postfix = [ '_quadratic_sm_5_rsl.txt' ];
            for j = 1 : size(case_num_cont, 1)
                fprintf([ case_num_cont{j} ' : ' ]);
                prefix_path = [ data_dir '/' case_num_cont{j} '/' Prefix_cont '_' case_num_cont{j} ];
                postfix_surf = '_native_t1_';
                
                % Intensity-based features: corrected RI, pg, tg
                for i = 2 : NumIntSurf+2
                    if(i == 1)
                    ...
                    T1_PG_IntCortical_temp(i, 1:vertexnum, j) = SurfStatReadData( { [ basename{1} 'pg_abs_dist' file_postfix ], [ basename{2} 'pg_abs_dist' file_postfix ] } );                    
                end
                fprintf([ 'T1: PG_abs_dist in Inc\n' ]);                
        ...
        for FLAIR_read_file = 1
            ...
            for j = 1 : size(case_num_cont, 1)
                ...
                for i = 2 : NumIntSurf+2
                    ...
                    FLAIR_PG_IntCortical_temp(i, 1:vertexnum, j) = SurfStatReadData( { [ basename{1} 'pg_abs_dist' file_postfix ], [ basename{2} 'pg_abs_dist' file_postfix ] } );                    
                end                
                fprintf([ 'FLAIR: PG_abs_dist in Inc\n' ]);                
            end
    ...
    for patient_computation = 1
        ...
        for T1_read_file = 1
            ...
            for j = 1 : size(case_num_pat, 1)
                ...
                prefix_path = [ data_dir '/' case_num_pat{j} '/' Prefix_pat '_' case_num_pat{j} ];
                postfix_surf = '_native_t1_';
                ...
                for i = 2 : NumIntSurf+2
                    ...
                    T1_PG_IntCortical_temp2(i, 1:vertexnum, j) = SurfStatReadData( { [ basename{1} 'pg_abs_dist' file_postfix ], [ basename{2} 'pg_abs_dist' file_postfix ] } );                    
                end
                fprintf([ 'T1: PG_abs_dist in Inc\n' ]);
        ...
        for FLAIR_read_file = 1
            ...
            for j = 1 : size(case_num_pat, 1)
                ...
                for i = 2 : NumIntSurf+2
                    ...
                    FLAIR_PG_IntCortical_temp2(i, 1:vertexnum, j) = SurfStatReadData( { [ basename{1} 'pg_abs_dist' file_postfix ], [ basename{2} 'pg_abs_dist' file_postfix ] } );                    
                end
                fprintf([ 'FLAIR: PG_abs_dist in Inc\n' ]);                
...
for lesional_feature = 1
    ...
    for lesion_feature_extraction = 1
        ...
        mean_z_lesion = zeros(10, size(case_num_pat, 1));
        std_z_lesion  = zeros(10, size(case_num_pat, 1));
        ...
        for j = 1 : size(case_num_pat_temp, 1)
            ...
            if(exist([ lesion_label_dir '/' Prefix_pat '_' case_num_pat_temp{j} '_label_union_left_rsl.txt' ], 'file'))
                ...
                mean_z_lesion(1:5, j) = mean(T1_PG_IntCortical_z(:, lesion_label_v, idx), 2);
                std_z_lesion(1:5, j)  = std(T1_PG_IntCortical_z(:, lesion_label_v, idx), 0, 2);                
                
                mean_z_lesion(6:10, j) = mean(FLAIR_PG_IntCortical_z(:, lesion_label_v, idx), 2);
                std_z_lesion(6:10, j)  = std(FLAIR_PG_IntCortical_z(:, lesion_label_v, idx), 0, 2);                
            else
        ...
        % controls        
        mean_z_lesion_cont = zeros(10, size(case_num_pat, 1), size(case_num_cont_temp, 1));
        std_z_lesion_cont  = zeros(10, size(case_num_pat, 1), size(case_num_cont_temp, 1));        
        for j = 1 : size(case_num_pat_temp, 1)
            idx = find(strcmp(case_num_pat_temp, case_num_pat_temp{j}));
            ...
            if(exist([ lesion_label_dir '/' Prefix_pat '_' case_num_pat_temp{j} '_label_union_left_rsl.txt' ], 'file'))
                ...
                for c = 1 : size(case_num_cont_temp, 1)
                    %% Cortical T1 intensity
                    mean_z_lesion_cont(1:5, j, c) = mean(T1_PG_IntCortical_z_cont(:, lesion_label_v, c), 2);
                    std_z_lesion_cont(1:5, j, c)  = std(T1_PG_IntCortical_z_cont(:, lesion_label_v, c), 0, 2);                    
                    
                    mean_z_lesion_cont(6:10, j, c) = mean(FLAIR_PG_IntCortical_z_cont(:, lesion_label_v, c), 2);
                    std_z_lesion_cont(6:10, j, c)  = std(FLAIR_PG_IntCortical_z_cont(:, lesion_label_v, c), 0, 2);                    
                end
            else
                disp([ case_num_pat_temp{j} ' does not have a lesion label file' ]);
            end
