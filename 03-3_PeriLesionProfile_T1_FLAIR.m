...
for read_matfiles = 1
    ...
    if patient_comp == 1
        load(['zscore_database_perilesional_T1_FLAIR_sm_' num2str(Kernel) '.mat']);
    ...
    if control_comp == 1
        ...
        load(['zscore_database_perilesional_T1_FLAIR_sm_' num2str(Kernel) '_cont.mat']);            
    ...
    if disease_control_comp == 1
        ...
        load(['zscore_database_perilesional_T1_FLAIR_sm_' num2str(Kernel) '_disease_cont.mat']);
...
for feature_extraction = 1
    ...
    if control_comp == 1
        ...
        feature_table = [];
        feature_parameters = [
            { 'T1_RI_IntCortical_z_cont', 5, 1 };           %% 01 Int RI     1-5
            { 'T1_PG_IntCortical_z_cont', 5, 1 };           %% 02 PG         6-10
            { 'T1_PG_gw_IntCortical_z_cont', 1, 5 };        %% 03 GM_WM PG   11
            { 'T1_TG_IntCortical_z_cont', 5, 1 };           %% 04 TG         12-16
            { 'T1_RI_SubCortical_z_cont', 3, 6 };           %% 05 Sub RI     17-19
            { 'T1_CT_midCortical_z_cont', 1, 3 };           %% 06 CT         20
            { 'T1_MC_midCortical_z_cont', 1, 3 };           %% 07 MC         21
            { 'T1_SD_wmCortical_z_cont',  1, 5 };           %% 08 SD         22
            { 'FLAIR_RI_IntCortical_z_cont', 5, 1 };        %% 09 Int RI     23-27
            { 'FLAIR_PG_IntCortical_z_cont', 5, 1 };        %% 10 PG         28-32
            { 'FLAIR_PG_gw_IntCortical_z_cont', 1, 5 };     %% 11 GM_WM PG   33
            { 'FLAIR_TG_IntCortical_z_cont', 5, 1 };        %% 12 TG         34-38
            { 'FLAIR_RI_SubCortical_z_cont', 3, 6 };        %% 13 Sub RI     39-41
            ];
        ...
        elseif(pl_feature_read_again == 1)
            ...
            for c = 1 : size(case_num_cont, 1)
                ...
                for k = 1 : size(feature_parameters, 1)
                    surf_num = feature_parameters{k, 2};
                    if(strfind(feature_parameters{k, 1}, 'T1'))
                        geoDist = geoDist_t1_cont_new;
                    elseif(strfind(feature_parameters{k, 1}, 'FLAIR'))
                        geoDist = geoDist_flair_cont_new;
                    end
                    disp(feature_parameters{k, 1});
    ...
    if disease_control_comp == 1
        ...
        feature_parameters = [
            { 'T1_RI_IntCortical_z_disease_cont', 5, 1 };           %% 01 Int RI     1-5
            { 'T1_PG_IntCortical_z_disease_cont', 5, 1 };           %% 02 PG         6-10
            { 'T1_PG_gw_IntCortical_z_disease_cont', 1, 5 };        %% 03 GM_WM PG   11
            { 'T1_TG_IntCortical_z_disease_cont', 5, 1 };           %% 04 TG         12-16
            { 'T1_RI_SubCortical_z_disease_cont', 3, 6 };           %% 05 Sub RI     17-19
            { 'T1_CT_midCortical_z_disease_cont', 1, 3 };           %% 06 CT         20
            { 'T1_MC_midCortical_z_disease_cont', 1, 3 };           %% 07 MC         21
            { 'T1_SD_wmCortical_z_disease_cont',  1, 5 };           %% 08 SD         22
            { 'FLAIR_RI_IntCortical_z_disease_cont', 5, 1 };        %% 09 Int RI     23-27
            { 'FLAIR_PG_IntCortical_z_disease_cont', 5, 1 };        %% 10 PG         28-32
            { 'FLAIR_PG_gw_IntCortical_z_disease_cont', 1, 5 };     %% 11 GM_WM PG   33
            { 'FLAIR_TG_IntCortical_z_disease_cont', 5, 1 };        %% 12 TG         34-38
            { 'FLAIR_RI_SubCortical_z_disease_cont', 3, 6 };        %% 13 Sub RI     39-41
            ];
        
        if(pl_feature_read_again == 0)
            load(['pl_feature_set_sm_' num2str(Kernel) '_T1_FLAIR_disease_cont.mat']);
        elseif(pl_feature_read_again == 1)
            pl_feature_set = [];
            for k = 1 : size(feature_parameters, 1)
                pl_feature_set(k).name = feature_parameters{k, 1};
            ...
            for c = 1 : size(case_num_disease_cont, 1)
                ...
                for k = 1 : size(feature_parameters, 1)
                    surf_num = feature_parameters{k, 2};
                    if(strfind(feature_parameters{k, 1}, 'T1'))
                        geoDist = geoDist_t1_disease_cont_new;
                    elseif(strfind(feature_parameters{k, 1}, 'FLAIR'))
                        geoDist = geoDist_flair_disease_cont_new;
                    end
                    disp(feature_parameters{k, 1});
...
%             p_value_set = [ p_value_set p_type_IIa p_type_IIb p_type_IIa_vs_IIb];
        ...
        for FLAIR_subcortical_intensity = 1
            ...
            FDR_threshold_final_perilesion_profiling = 0.0052;
    ...
    if control_comp == 1
        ...
        for T1_perpendicular_gradient = 1
            ...
            pl_feature_set_cont = pl_feature_set(feature_idx).data(:, :, surface_idx);
            feature_idx = 3; surface_idx = [1];
            pl_feature_set_cont(:, :, size(pl_feature_set_cont, 3)+1) = pl_feature_set(feature_idx).data(:, :, surface_idx);
            feature_type = mean(pl_feature_set(feature_idx).data(:, :, surface_idx), 3);                        
        ...
        for FLAIR_tangential_gradient = 1
            ...
            feature_idx = 12; surface_idx = [2:4];
            x_lim = [0 perilesional_distance+2];
            y_lim = [-2 2];
            sig = -2;            
