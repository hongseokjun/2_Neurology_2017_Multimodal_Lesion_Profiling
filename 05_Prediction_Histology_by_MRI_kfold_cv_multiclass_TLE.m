...
addpath(genpath([ '/local_raid/seokjun/01_project/08_Autism_abide/01_analysis/toolbox/mRMR_0.9_compiled' ]));
...
for read_matfiles = 1
    ...
    load('zscore_database_T1_FLAIR_disease_cont.mat');
    mean_z_lesion_T1_FLAIR_cont = mean_z_lesion_disease_cont;
    load('zscore_database_DTI_disease_cont.mat');
    mean_z_lesion_DTI_cont      = mean_z_lesion_disease_cont;
    load('zscore_database_fMRI_disease_cont.mat');
    mean_z_lesion_fMRI_cont     = mean_z_lesion_disease_cont;
...
for read_demodata_setup_params = 1
    ...
    Cases_disease_cont        = [ BASEDIR '/01_analysis/Demographic_data_TLE_final.txt' ];
    Group_cont                = 'control';
    Prefix_cont               = 'TLE';
...
for t1_flair_dti_fMRI = 1
    ...
    for prediction_using_multimodal_depth_features = 1
        ...
        for all_features_multimodal_multisurface_perilesional = 1
            ...
            keepout_vec = logical(zeros(size(mean_z_lesion_final, 1), 1));
            keepout_vec([ 1 6 10 11 23:28 29 34 38 39 48:53 54 55 57 59 61 63:65 66 68 69 71 73 75:77 80 81 ]) = 1;          
...
%                     fs([ 3 37 90 ]) = 1;
            ...
            % 10-folds
            % 89%: 2     8     9    11    13    15    18    20    22    23    25    27    29    30    33    35    37    47    48    50    60    61    64    66    70    71    72    73    74    77    87    89    95    
            find(mean(mMmSPeri_prediction_set, 1) == 42/47)
            iter = 66
            
            % 5-folds
            % 86%: 6     9    10    27    34    45    51    54    56    57    63    67    69    77    83    85    90    91    92    95
            find(mean(mMmSPeri_prediction_set, 1) == 40/47)
            iter = 83
            
            actual_smpsize = [ 24 9 24 ];
            ...
            std(sum((squeeze(sum(mMmSPeri_fs_set, 1)))>0, 1))       % STD number of features used for clasfication at each iteration
            min(sum((squeeze(sum(mMmSPeri_fs_set, 1)))>0, 1))       % min number of features used for clasfication at each iteration
            max(sum((squeeze(sum(mMmSPeri_fs_set, 1)))>0, 1))       % max number of features used for clasfication at each iteration
    ...
    for mcnemar_test = 1
        ...
        FCD_idx = [ (max(control_idx) + 1) : 47 ]';
        T1_median_idx = find(median(mean(T1_prediction_set, 1)) == mean(T1_prediction_set, 1)); % T1_median_idx = find((mean(T1_prediction_set, 1) - 0.7872)<0.001);
        ...
        mMGlobalf_median_idx = find(abs(median(mean(mMGlobalf_prediction_set, 1)) - mean(mMGlobalf_prediction_set, 1))<0.01);
        mMmS_median_idx = find(median(mean(mMmS_prediction_set, 1)) == mean(mMmS_prediction_set, 1));
        mMmSPeri_median_idx = find(median(mean(mMmSPeri_prediction_set, 1)) == mean(mMmSPeri_prediction_set, 1)); % mMmSPeri_median_idx = find(mean(mMmSPeri_prediction_set, 1) == 41/47);
