...
for zscore_computation = 1
    ...
    for control_computation = 1
        ...
        for DTI_outlier_detection = 1
            ...
            vid_max = 100;
            sub_idx = zeros(size(case_num_cont, 1), 24);
        ...
        for DTI_zscore_cont = 1
            
            case_num_cont_temp = case_num_cont(case_cont_real); 
            
            %% z-score
            DTI_FA_IntCortical_z_cont         = zeros(NumIntSurf+2, vertexnum, size(case_num_cont_temp, 1));
            DTI_FA_SubCortical_z_cont          = zeros(NumSubSurf,   vertexnum, size(case_num_cont_temp, 1));
            DTI_MD_IntCortical_z_cont          = zeros(NumIntSurf+2, vertexnum, size(case_num_cont_temp, 1));
            DTI_MD_SubCortical_z_cont          = zeros(NumSubSurf,   vertexnum, size(case_num_cont_temp, 1));
            ...
            for j = 1 : size(case_num_cont_temp, 1)
                ...
                %% Intensity-based features: corrected RI, pg, tg
                DTI_FA_IntCortical_z_cont(:, 1:vertexnum, j) = (DTI_FA_IntCortical_temp(:, 1:vertexnum, case_cont_real(j)) - DTI_FA_IntCortical_mean) ./ DTI_FA_IntCortical_std;
                DTI_MD_IntCortical_z_cont(:, 1:vertexnum, j) = (DTI_MD_IntCortical_temp(:, 1:vertexnum, case_cont_real(j)) - DTI_MD_IntCortical_mean) ./ DTI_MD_IntCortical_std;
                fprintf([ 'z-score of FA and MD in Inc, ' ]);
                
                %% Intensity-based features of subcortical surfaces: RI, pg, tg
                DTI_FA_SubCortical_z_cont(:, 1:vertexnum, j) = (DTI_FA_SubCortical_temp(:, 1:vertexnum, case_cont_real(j)) - DTI_FA_SubCortical_mean) ./ DTI_FA_SubCortical_std;
                DTI_MD_SubCortical_z_cont(:, 1:vertexnum, j) = (DTI_MD_SubCortical_temp(:, 1:vertexnum, case_cont_real(j)) - DTI_MD_SubCortical_mean) ./ DTI_MD_SubCortical_std;
...
for lesional_feature = 1
    ...
    for lesion_feature_extraction = 1
        ...
        mean_z_lesion = zeros(24, size(case_num_pat, 1));
        std_z_lesion  = zeros(24, size(case_num_pat, 1));
        ...
        for j = 1 : size(case_num_pat_temp, 1)
            ...
            if(exist([ lesion_label_dir '/' Prefix_pat '_' case_num_pat_temp{j} '_label_union_left_rsl.txt' ], 'file'))
                ...
                mean_z_lesion(1:5, j) = mean(DTI_FA_IntCortical_z(:, lesion_label_v, idx), 2);
                std_z_lesion(1:5, j)  = std(DTI_FA_IntCortical_z(:, lesion_label_v, idx), 0, 2);                
                mean_z_lesion(6:12, j) = mean(DTI_FA_SubCortical_z(:, lesion_label_v, idx), 2);
                std_z_lesion(6:12, j)  = std(DTI_FA_SubCortical_z(:, lesion_label_v, idx), 0, 2);
                mean_z_lesion(13:17, j) = mean(DTI_MD_IntCortical_z(:, lesion_label_v, idx), 2);
                std_z_lesion(13:17, j)  = std(DTI_MD_IntCortical_z(:, lesion_label_v, idx), 0, 2);
                mean_z_lesion(18:24, j) = mean(DTI_MD_SubCortical_z(:, lesion_label_v, idx), 2);
                std_z_lesion(18:24, j)  = std(DTI_MD_SubCortical_z(:, lesion_label_v, idx), 0, 2);
            else
        ...
        % controls
        mean_z_lesion_cont = zeros(24, size(case_num_pat, 1), size(case_num_cont_temp, 1));
        std_z_lesion_cont  = zeros(24, size(case_num_pat, 1), size(case_num_cont_temp, 1));        
        for j = 1 : size(case_num_pat_temp, 1)
            idx = find(strcmp(case_num_pat_temp, case_num_pat_temp{j}));
            ...
            if(exist([ lesion_label_dir '/' Prefix_pat '_' case_num_pat_temp{j} '_label_union_left_rsl.txt' ], 'file'))
                ...
                for c = 1 : size(case_num_cont_temp, 1)
                    %% FA MD Intra/Subcortical surfaces
                    mean_z_lesion_cont(1:5, j, c) = mean(DTI_FA_IntCortical_z_cont(:, lesion_label_v, c), 2);
                    std_z_lesion_cont(1:5, j, c)  = std(DTI_FA_IntCortical_z_cont(:, lesion_label_v, c), 0, 2);
                    mean_z_lesion_cont(6:12, j, c) = mean(DTI_FA_SubCortical_z_cont(:, lesion_label_v, c), 2);
                    std_z_lesion_cont(6:12, j, c)  = std(DTI_FA_SubCortical_z_cont(:, lesion_label_v, c), 0, 2);
                    mean_z_lesion_cont(13:17, j, c) = mean(DTI_MD_IntCortical_z_cont(:, lesion_label_v, c), 2);
                    std_z_lesion_cont(13:17, j, c)  = std(DTI_MD_IntCortical_z_cont(:, lesion_label_v, c), 0, 2);                    
                    mean_z_lesion_cont(18:24, j, c) = mean(DTI_MD_SubCortical_z_cont(:, lesion_label_v, c), 2);
                    std_z_lesion_cont(18:24, j, c)  = std(DTI_MD_SubCortical_z_cont(:, lesion_label_v, c), 0, 2);
                end
            else
                disp([ case_num_pat_temp{j} ' does not have a lesion label file' ]);
            end
    ...
    for save_zscore = 1
        ...
        if(~exist('zscore_database_DTI.mat', 'file'))
            save('zscore_database_DTI.mat', ... 
                 'DTI_FA_IntCortical_z', 'DTI_MD_IntCortical_z', 'DTI_FA_SubCortical_z', 'DTI_MD_SubCortical_z', ...
                 'mean_z_lesion', 'std_z_lesion', ...
                 'case_num_cont', 'case_num_pat', 'case_cont_real', 'case_num_pat_temp', ...
                 'histo_type');
        end
        
        if(~exist('zscore_database_DTI_cont.mat', 'file'))
            save('zscore_database_DTI_cont.mat', ... 
                 'DTI_FA_IntCortical_z_cont', 'DTI_MD_IntCortical_z_cont', 'DTI_FA_SubCortical_z_cont', 'DTI_MD_SubCortical_z_cont', ...
