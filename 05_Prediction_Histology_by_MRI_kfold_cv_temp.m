...
addpath('/media/LaCie/seokjun/03_downloads/LSSVMlabv1_8_R2009b_R2011a');
...
for read_demodata_setup_params = 1
    ...
    OUTPATH = '/media/LaCie/seokjun/01_project/04_IntracorticalAnalysis/03_result/prediction_histology_by_MRI/';
    Cases_cont                = '/media/LaCie/seokjun/01_project/04_IntracorticalAnalysis/01_analysis/Demographic_data_control.txt';
    Group_cont                = 'control';
    Prefix_cont               = 'TLE';
    
    Cases_pat                 = '/media/LaCie/seokjun/01_project/04_IntracorticalAnalysis/01_analysis/Demographic_data_FCD.txt';
    Cases_pat_dti             = '/media/LaCie/seokjun/01_project/04_IntracorticalAnalysis/01_analysis/Demographic_data_FCD_DTI.txt';
    Cases_pat_fMRI            = '/media/LaCie/seokjun/01_project/04_IntracorticalAnalysis/01_analysis/Demographic_data_FCD_fMRI.txt';
    Group_pat                 = 'FCD';
    Prefix_pat                = 'mcd';
    ...
    average_surface_dir       = '/media/LaCie/seokjun/01_project/04_IntracorticalAnalysis/03_result/average_surfaces/';
    
    visualization = 1;
    save_fig = 0;
    load('/media/LaCie/seokjun/01_project/04_IntracorticalAnalysis/01_analysis/pipeline/postprocessing_zscore_visualization/colormap_noel1.mat');
...
for t1_flair_dti_fMRI = 1
    ...
    for featureset_build = 1
        ...
        mean_z_lesion_temp1 = [ mean_z_lesion_T1_FLAIR(:, ia1); mean_z_lesion_DTI(:, ia2); mean_z_lesion_fMRI; ]; 
        mean_z_lesion_temp1(79, :) = (1/3)*mean_z_lesion_fMRI(2, :) + (2/3)*pl_feature_set_fMRI(2).data(peri_lesional_dist_fMRI(1), :);        
    ...
    for prediction_using_multimodal_depth_features = 1
        ...
        for all_modalities_together = 1
            ...
            discriminative_features_set = [];
            opts = statset();
            mean_z_lesion_final = mean_z_lesion_temp1;
            ...
            kfold = 10;
            classifier = 1;
            FCDTypeIIA = strcmp(histo_type_fMRI, 'FCDIIa');
            FCDTypeIIB = strcmp(histo_type_fMRI, 'FCDIIb');
            
            prediction_set = zeros(length(histo_type_fMRI), iteration_num);
            fs_set = zeros(iteration_num, size(mean_z_lesion_final, 1));
            kfold_set = zeros(iteration_num, length(histo_type_fMRI));
            for i = 1 : iteration_num
                
                Indices = crossvalind('kfold', histo_type_fMRI, kfold);
                kfold_set(i, :) = Indices';
                
                prediction_curr = zeros(length(histo_type_fMRI), 1);
                ...
                parfor c = 1 : kfold
                    ...
                    train_groupA = find(strcmp(histo_type_fMRI, 'FCDIIa').*train_cases);
                    train_groupB = find(strcmp(histo_type_fMRI, 'FCDIIb').*train_cases);
                    
                    CV_group = histo_type_fMRI;
                    CV_group(test_cases) = [];
                    ...
                    if(classifier == 1)
                        [fs,history] = sequentialfs(@fs_func_svm,mean_z_lesion_temp_lvo',CV_group,'cv', p, 'direction', 'forward', 'keepout', keepout_vec', 'options',opts);
                    elseif(classifier == 2)
                        [fs,history] = sequentialfs(@fs_func_svm_fitc, mean_z_lesion_temp_lvo',CV_group,'cv', p, 'direction', 'forward', 'keepout', keepout_vec', 'options',opts);
                    end
                    fs_set_temp{c} = fs; 
                ...
                for c = 1 : kfold
                    ...
                    num_of_each_group = min( [ num_of_FCDIIA_in_train num_of_FCDIIB_in_train] );                    
                    ...
                    CV_group_test = histo_type_fMRI(test_real);                         
                    ...
                    if(bagging_num==0)
                        ...
                        CV_group_train = histo_type_fMRI(train_real);
                        CV_group_test = histo_type_fMRI(test_real);
                        
                        training_data = mean_z_lesion_final(fs, train_real);
                        test_data     = mean_z_lesion_final(fs, test_real);
                        
                        discriminative_features_set = [ discriminative_features_set; fs];
                        
                        if(classifier==1)
                            
                            SVMstruct = svmtrain(training_data', CV_group_train, 'Kernel_Function', 'polynomial', 'boxconstraint', 100);
                            label = svmclassify(SVMstruct, test_data');
                            
                        elseif(classifier==2)
                            
                            SVMstruct = fitcsvm(training_data', CV_group_train, 'KernelFunction', 'polynomial');
                            label = predict(SVMstruct,test_data');
                            
                        end
                        
                        test_bagging = strcmp(label, 'FCDIIa');                        
                        prediction_curr(test_cases) = test_bagging == strcmp(CV_group_test, 'FCDIIa');
                    ...
                    elseif(bagging_num>0)
                        ...
                        parfor j = 1 : bagging_num
                            ...
                            if(num_of_FCDIIA_in_train >= num_of_FCDIIB_in_train)
                                
                                train_groupA_real = sort(train_groupA(randperm(num_of_FCDIIA_in_train, num_of_FCDIIB_in_train)));
                                train_groupB_real = train_groupB;
                                
                            elseif(num_of_FCDIIB_in_train > num_of_FCDIIA_in_train)
                                
                                train_groupA_real = train_groupA;
                                train_groupB_real = sort(train_groupB(randperm(num_of_FCDIIB_in_train, num_of_FCDIIA_in_train)));
                                
                            end
                            
                            train_real = sort([ train_groupA_real; train_groupB_real]);
                            test_real  = find(test_cases);
                            ...
                            test_bagging{j} = strcmp(label, 'FCDIIa');
                            
                        end
                        
                        prediction_curr(test_cases) = mode(double(cell2mat(test_bagging)), 2) == strcmp(CV_group_test, 'FCDIIa');
                        
                    end
                    
                end
                    
                disp([ 'iter ', num2str(i), ' : ', num2str(sum(prediction_curr)/length(histo_type_fMRI))]);
                prediction_set(:, i) = prediction_curr;
                
            end
            
            sensitivity = zeros(iteration_num, 1);
            specificity = zeros(iteration_num, 1);
            for i = 1 : iteration_num
                sensitivity(i) = sum(prediction_set(FCDTypeIIA, i)==1)/sum(FCDTypeIIA);                
                specificity(i) = sum(prediction_set(FCDTypeIIB, i)==1)/sum(FCDTypeIIB);                
            end
            
            % result
            % 10-fold validation with bagging    : 85±6% (sensitivity:87±10%, specificity: 84±8%)
            % 10-fold validation without bagging : 85±6% 
            % leave-one-out with bagging         : 88±5%
            % leave-one-out without bagging      : 91%
        ...
        for all_features_including_perilesional_ab = 1
                ...
                for c = 1 : kfold
                    ...
                    if(bagging_num==0)
                        ...
                        if(classifier==1)
                            ...
                            SVMstruct = svmtrain(training_data', CV_group_train, 'Kernel_Function', 'polynomial');
                            label = svmclassify(SVMstruct, test_data');
            ...
            % result
            % 10-fold validation with bagging    : 89±7% (sensitivity:92±9%, specificity: 87±9%)
            % 10-fold validation without bagging : 87±7% (sensitivity:91±7%, specificity: 80±11%)
            % leave-one-out with bagging         : 93±5% (sensitivity:96±6%, specificity: 90±5%)
            % leave-one-out without bagging      : 95% (sensitivity: 100%, specificity: 95%)
