...
for lesional_feature = 1
    ...
    for lesion_feature_extraction_raw = 1
        
        % patients
        lesion_label_dir = '/data/noel/noel6/CTFCD-1.2.0_64/Lesion/lesion_surf/';
        mean_raw_lesion = zeros(53, size(case_num_pat, 1));
        std_raw_lesion  = zeros(53, size(case_num_pat, 1));
        case_num_pat_temp = case_num_pat;
        for j = 1 : size(case_num_pat_temp, 1)
            idx = find(strcmp(case_num_pat_temp, case_num_pat_temp{j}));
            ...
            if(exist([ lesion_label_dir '/' Prefix_pat '_' case_num_pat_temp{j} '_label_union_left_rsl.txt' ], 'file'))
                ...
                lesion_label_v = find(lesion_label_data ~= 0);
                %% Cortical T1 intensity
                mean_raw_lesion(1:5, j) = mean(T1_RI_IntCortical_temp2(:, lesion_label_v, idx), 2);
                std_raw_lesion(1:5, j)  = std(T1_RI_IntCortical_temp2(:, lesion_label_v, idx), 0, 2);
                mean_raw_lesion(6:10, j) = mean(T1_PG_IntCortical_temp2(:, lesion_label_v, idx), 2);
                std_raw_lesion(6:10, j)  = std(T1_PG_IntCortical_temp2(:, lesion_label_v, idx), 0, 2);
                mean_raw_lesion(11:15, j) = mean(T1_TG_IntCortical_temp2(:, lesion_label_v, idx), 2);
                std_raw_lesion(11:15, j)  = std(T1_TG_IntCortical_temp2(:, lesion_label_v, idx), 0, 2);
                mean_raw_lesion(16, j) = mean(T1_PG_gw_IntCortical_temp2(idx, lesion_label_v), 2);
                std_raw_lesion(16, j)  = std(T1_PG_gw_IntCortical_temp2(idx, lesion_label_v), 0, 2);
                
                %% T1 Morphological metrics
                mean_raw_lesion(17, j) = mean(T1_CT_midCortical_temp2(idx, lesion_label_v), 2);
                std_raw_lesion(17, j)  = std(T1_CT_midCortical_temp2(idx, lesion_label_v), 0, 2);
                mean_raw_lesion(18, j) = mean(T1_MC_midCortical_temp2(idx, lesion_label_v), 2);
                std_raw_lesion(18, j)  = std(T1_MC_midCortical_temp2(idx, lesion_label_v), 0, 2);
                mean_raw_lesion(19, j) = mean(T1_SD_wmCortical_temp2(idx, lesion_label_v), 2);
                std_raw_lesion(19, j)  = std(T1_SD_wmCortical_temp2(idx, lesion_label_v), 0, 2);
                
                %% subcortical T1 intensity
                mean_raw_lesion(20:22, j) = mean(T1_RI_SubCortical_temp2(:, lesion_label_v, idx), 2);
                std_raw_lesion(20:22, j)  = std(T1_RI_SubCortical_temp2(:, lesion_label_v, idx), 0, 2);
                mean_raw_lesion(23:25, j) = mean(T1_PG_SubCortical_temp2(:, lesion_label_v, idx), 2);
                std_raw_lesion(23:25, j)  = std(T1_PG_SubCortical_temp2(:, lesion_label_v, idx), 0, 2);
                mean_raw_lesion(26:28, j) = mean(T1_TG_SubCortical_temp2(:, lesion_label_v, idx), 2);
                std_raw_lesion(26:28, j)  = std(T1_TG_SubCortical_temp2(:, lesion_label_v, idx), 0, 2);
                
                %% Cortical FLAIR intensity
                mean_raw_lesion(29:33, j) = mean(FLAIR_RI_IntCortical_temp2(:, lesion_label_v, idx), 2);
                std_raw_lesion(29:33, j)  = std(FLAIR_RI_IntCortical_temp2(:, lesion_label_v, idx), 0, 2);
                mean_raw_lesion(34:38, j) = mean(FLAIR_PG_IntCortical_temp2(:, lesion_label_v, idx), 2);
                std_raw_lesion(34:38, j)  = std(FLAIR_PG_IntCortical_temp2(:, lesion_label_v, idx), 0, 2);
                mean_raw_lesion(39:43, j) = mean(FLAIR_TG_IntCortical_temp2(:, lesion_label_v, idx), 2);
                std_raw_lesion(39:43, j)  = std(FLAIR_TG_IntCortical_temp2(:, lesion_label_v, idx), 0, 2);
                mean_raw_lesion(44, j) = mean(FLAIR_PG_gw_IntCortical_temp2(idx, lesion_label_v), 2);
                std_raw_lesion(44, j)  = std(FLAIR_PG_gw_IntCortical_temp2(idx, lesion_label_v), 0, 2);
                
                %% subcortical FLAIR intensity
                mean_raw_lesion(45:47, j) = mean(FLAIR_RI_SubCortical_temp2(:, lesion_label_v, idx), 2);
                std_raw_lesion(45:47, j)  = std(FLAIR_RI_SubCortical_temp2(:, lesion_label_v, idx), 0, 2);
                mean_raw_lesion(48:50, j) = mean(FLAIR_PG_SubCortical_temp2(:, lesion_label_v, idx), 2);
                std_raw_lesion(48:50, j)  = std(FLAIR_PG_SubCortical_temp2(:, lesion_label_v, idx), 0, 2);
                mean_raw_lesion(51:53, j) = mean(FLAIR_TG_SubCortical_temp2(:, lesion_label_v, idx), 2);
                std_raw_lesion(51:53, j)  = std(FLAIR_TG_SubCortical_temp2(:, lesion_label_v, idx), 0, 2);
            else
                disp([ case_num_pat_temp{j} ' does not have a lesion label file' ]);
            end
        end
        ...
        % controls        
        mean_raw_lesion_cont = zeros(53, size(case_num_pat, 1), size(case_num_cont_temp, 1));
        std_raw_lesion_cont  = zeros(53, size(case_num_pat, 1), size(case_num_cont_temp, 1));        
        for j = 1 : size(case_num_pat_temp, 1)
            idx = find(strcmp(case_num_pat_temp, case_num_pat_temp{j}));
            ...
            if(exist([ lesion_label_dir '/' Prefix_pat '_' case_num_pat_temp{j} '_label_union_left_rsl.txt' ], 'file'))
                ...
                for c = 1 : size(case_num_cont_temp, 1)
                    %% Cortical T1 intensity
                    mean_raw_lesion_cont(1:5, j, c) = mean(T1_RI_IntCortical_temp(:, lesion_label_v, c), 2);
                    std_raw_lesion_cont(1:5, j, c)  = std(T1_RI_IntCortical_temp(:, lesion_label_v, c), 0, 2);
                    mean_raw_lesion_cont(6:10, j, c) = mean(T1_PG_IntCortical_temp(:, lesion_label_v, c), 2);
                    std_raw_lesion_cont(6:10, j, c)  = std(T1_PG_IntCortical_temp(:, lesion_label_v, c), 0, 2);
                    mean_raw_lesion_cont(11:15, j, c) = mean(T1_TG_IntCortical_temp(:, lesion_label_v, c), 2);
                    std_raw_lesion_cont(11:15, j, c)  = std(T1_TG_IntCortical_temp(:, lesion_label_v, c), 0, 2);
                    mean_raw_lesion_cont(16, j, c) = mean(T1_PG_gw_IntCortical_temp(c, lesion_label_v), 2);
                    std_raw_lesion_cont(16, j, c)  = std(T1_PG_gw_IntCortical_temp(c, lesion_label_v), 0, 2);
                    
                    %% T1 Morphological metrics
                    mean_raw_lesion_cont(17, j, c) = mean(T1_CT_midCortical_temp(c, lesion_label_v), 2);
                    std_raw_lesion_cont(17, j, c)  = std(T1_CT_midCortical_temp(c, lesion_label_v), 0, 2);
                    mean_raw_lesion_cont(18, j, c) = mean(T1_MC_midCortical_temp(c, lesion_label_v), 2);
                    std_raw_lesion_cont(18, j, c)  = std(T1_MC_midCortical_temp(c, lesion_label_v), 0, 2);
                    mean_raw_lesion_cont(19, j, c) = mean(T1_SD_wmCortical_temp(c, lesion_label_v), 2);
                    std_raw_lesion_cont(19, j, c)  = std(T1_SD_wmCortical_temp(c, lesion_label_v), 0, 2);
                    
                    %% subcortical T1 intensity
                    mean_raw_lesion_cont(20:22, j, c) = mean(T1_RI_SubCortical_temp(:, lesion_label_v, c), 2);
                    std_raw_lesion_cont(20:22, j, c)  = std(T1_RI_SubCortical_temp(:, lesion_label_v, c), 0, 2);
                    mean_raw_lesion_cont(23:25, j, c) = mean(T1_PG_SubCortical_temp(:, lesion_label_v, c), 2);
                    std_raw_lesion_cont(23:25, j, c)  = std(T1_PG_SubCortical_temp(:, lesion_label_v, c), 0, 2);
                    mean_raw_lesion_cont(26:28, j, c) = mean(T1_TG_SubCortical_temp(:, lesion_label_v, c), 2);
                    std_raw_lesion_cont(26:28, j, c)  = std(T1_TG_SubCortical_temp(:, lesion_label_v, c), 0, 2);
                    
                    %% Cortical FLAIR intensity
                    mean_raw_lesion_cont(29:33, j, c) = mean(FLAIR_RI_IntCortical_temp(:, lesion_label_v, c), 2);
                    std_raw_lesion_cont(29:33, j, c)  = std(FLAIR_RI_IntCortical_temp(:, lesion_label_v, c), 0, 2);
                    mean_raw_lesion_cont(34:38, j, c) = mean(FLAIR_PG_IntCortical_temp(:, lesion_label_v, c), 2);
                    std_raw_lesion_cont(34:38, j, c)  = std(FLAIR_PG_IntCortical_temp(:, lesion_label_v, c), 0, 2);
                    mean_raw_lesion_cont(39:43, j, c) = mean(FLAIR_TG_IntCortical_temp(:, lesion_label_v, c), 2);
                    std_raw_lesion_cont(39:43, j, c)  = std(FLAIR_TG_IntCortical_temp(:, lesion_label_v, c), 0, 2);
                    mean_raw_lesion_cont(44, j, c) = mean(FLAIR_PG_gw_IntCortical_temp(c, lesion_label_v), 2);
                    std_raw_lesion_cont(44, j, c)  = std(FLAIR_PG_gw_IntCortical_temp(c, lesion_label_v), 0, 2);
                    
                    %% subcortical FLAIR intensity
                    mean_raw_lesion_cont(45:47, j, c) = mean(FLAIR_RI_SubCortical_temp(:, lesion_label_v, c), 2);
                    std_raw_lesion_cont(45:47, j, c)  = std(FLAIR_RI_SubCortical_temp(:, lesion_label_v, c), 0, 2);
                    mean_raw_lesion_cont(48:50, j, c) = mean(FLAIR_PG_SubCortical_temp(:, lesion_label_v, c), 2);
                    std_raw_lesion_cont(48:50, j, c)  = std(FLAIR_PG_SubCortical_temp(:, lesion_label_v, c), 0, 2);
                    mean_raw_lesion_cont(51:53, j, c) = mean(FLAIR_TG_SubCortical_temp(:, lesion_label_v, c), 2);
                    std_raw_lesion_cont(51:53, j, c)  = std(FLAIR_TG_SubCortical_temp(:, lesion_label_v, c), 0, 2);
                end
            else
                disp([ case_num_pat_temp{j} ' does not have a lesion label file' ]);
            end
